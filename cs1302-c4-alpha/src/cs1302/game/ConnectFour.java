...
public class ConnectFour {
    ...
    public ConnectFour(int rows, int cols)  {
        ...
        if (rows < 6 || rows > 9 || cols < 7 || cols >  9) {
            ...
            throw new IllegalArgumentException("Values for rows and columns not supported");
    ...
    /**
     * Print debug statements to check code.
     */
    ...
    public void printDebugStatements () {
        ...
        System.out.println("Last Drop Row: " + lastDropRow);
        System.out.println("Last Dropped Col: " + lastDropCol);
        System.out.println("Phase: " + phase);
        System.out.println("Player token 0: " + player[0]);
        System.out.println("Player token 1: " + player[1]);
    ...
    public Token getTokenAt(int row, int col) {
        ...
        if (( row >= 0 && row < this.rows) && (col < this.cols && col >= 0)) {
            if (grid[row][col] == this.player[0]) {
                return this.player[0];
            } else if (grid[row][col] == this.player[1]) {
                return this.player[1];
            } else {
                return null;
            }
        } else {
            throw new IndexOutOfBoundsException("The specified row/column not within game grid.") ;
        }
    } // getTokenAt
    ...
    public void setPlayerTokens(Token token0, Token token1) {
        ...
        if ( token0 == token1 ) {
            throw new IllegalArgumentException("Both players given same token!");
        }
        if (getPhase() == GamePhase.PLAYABLE || getPhase() == GamePhase.OVER) {
            throw new IllegalStateException("Cannot set tokens in PLAYABLE or OVER game phases.");
        } else {
    ...
    public Token getPlayerToken(int player) {
        ...
        if (getPhase() == GamePhase.NEW) {
            throw new IllegalStateException("Cannot get player token in NEW game phase.");
        }
        if (player == 0 || player == 1) {
            return this.player[player];
        } else {
            throw new IllegalArgumentException("Player does not have valid token!");
        }
    ...
    public int getNumDropped() {
        ...
        if (this.phase == GamePhase.NEW || this.phase == GamePhase.READY) {
            throw new IllegalStateException("Can't return in current game phase!");
        }
        return numDropped;
    } // getNumDropped
    ...
    public int getLastDropRow() {
        ...
        if (this.phase == GamePhase.NEW || this.phase == GamePhase.READY) {
            throw new IllegalStateException("Cannot play with current game phase!");
        }
    ...
    public void dropToken(int player, int col) {
        ...
        // replace the entire contents of this method with your implementation
        //
        //loop through given column in grid and if row in col is null drop token there
        if ((col > this.cols) || (col < 0)) {
            throw new IndexOutOfBoundsException("column is out of bounds!");
        }

        if (player != 0 && player != 1) {
            throw new IllegalArgumentException("not a valid player token!");
        }
        //ILLEGAL STATE EXCEPTION IF SPECIFED COLUMN IN THE GRID IS FULL
        if (grid[0][col] == this.player[0] || (grid[0][col] == this.player[1])) {
            throw new IllegalStateException("Specified column in grid is full");
        }

        if (getPhase() != GamePhase.PLAYABLE && getPhase() != GamePhase.READY) {
            throw new IllegalStateException("player cannot drop token in current phase!");
        }

       //if
        //throw illegal state if specified column in grid is full --> add for loop to check
        this.phase = GamePhase.PLAYABLE;
        lastDropCol = col;
        for (int i = this.rows - 1; i >= 0; i--) {
            if ((grid[i][col] != this.player[0]) && (grid[i][col] != this.player[1])) {
                grid[i][col] = this.player[player];
                lastDropRow = i;
                numDropped++;
                break;
            }
    ...
    public boolean isLastDropConnectFour() {
        if (numDropped == (this.rows * this.cols)) {
            this.phase = GamePhase.OVER;
        }
        boolean verticalPossible = true; //VERTICAL WIN
        int vertCounter = 1, a = lastDropRow + 1;
        while (verticalPossible && vertCounter < 4) {
            if (a >= this.rows)  {
                verticalPossible = false;
            } else if (grid[lastDropRow][lastDropCol] == grid[a][lastDropCol]) {
                vertCounter++;
                a++;
            } else {
                verticalPossible = false;
        ...
        boolean reverseHorizontal = true, forwardCheck = true; //HORIZONTAL WIN
        int horizCounter = 1, p = lastDropCol - 1, q = lastDropCol + 1;
        while (reverseHorizontal && (horizCounter < 4)) {
            if (p < 0) {
                reverseHorizontal = false;
            } else if (grid[lastDropRow][lastDropCol] == grid[lastDropRow][p]) {
                horizCounter++;
                p--;
            } else {
                reverseHorizontal = false;
        ...
        while ( forwardCheck && (horizCounter < 4)) {
            if (q >= this.cols) {
                forwardCheck = false;
            } else if (grid[lastDropRow][lastDropCol] == grid[lastDropRow][q]) {
                horizCounter++;
                q++;
            } else {
                forwardCheck = false;
        ...
        if (vertCounter == 4 || horizCounter == 4 || diagonalWinner() || otherDiagonalWinner()) {
            this.phase = GamePhase.OVER;
            return true;
        } else {
            return false;
    ...
    //----------------------------------------------------------------------------------------------
     ...
     /**
     * Return {@code true} if the last token dropped via {@link #dropToken} created a
     * <em>connect four</em> diagonally with forward part down and reverse up.
     * The last dropped token creata a <em>connect four</em> diagonally, if there
     * are four equal tokens diagonally up to the right or up to the left.
     * @return {@code true} if the last token dropped created a <em>connect four</em>
     * diagonally with forward diagonal down and left part at top, else {@code false}
     */
    public boolean diagonalWinner() { //DIAGONAL WINNER
        boolean diagonalPossible = true, reverseUp = true, checkForwardDown = true;
        int diagCounterOne = 1;
        int i = lastDropRow - 1, j = lastDropCol - 1, t = lastDropRow + 1, r = lastDropCol + 1;
        while (reverseUp && (diagCounterOne < 4)) {
            if ((i < 0) || (j < 0)) {
                reverseUp = false;
            } else if (grid[lastDropRow][lastDropCol] == grid[i][j] ) {
                diagCounterOne++;
                if (diagCounterOne == 4) {
                    return true;
                }
        ...
        while (checkForwardDown && (diagCounterOne < 4)) {
            if (t == this.rows || r == this.cols) {
                checkForwardDown = false;
            } else  if (grid[lastDropRow][lastDropCol] == grid[t][r]) {
                diagCounterOne++;
                if (diagCounterOne == 4) {
                    return true;
                }
    ...
    /**
     * Return {@code true} if the last token dropped via {@link #dropToken} created a
     * <em>connect four</em> diagonally with forward up at the top.
     * The last dropped token creata a <em>connect four</em> diagonally, if there
     * are four equal tokens diagonally up to the right or up to the left.
     * @return {@code true} if the last token dropped created a <em>connect four</em>
     * diagonally from downward left to forward top right, else {@code false}
     */

    public boolean otherDiagonalWinner() {
    //close checkForwardDown
        int diagCounterTwo = 1;
        boolean forwardUp = true, reverseDown = true;
        int k = lastDropRow - 1, l = lastDropCol + 1, m = lastDropRow + 1, n = lastDropCol - 1;
        while (forwardUp && (diagCounterTwo < 4)) {
            if (k < 0 || l == this.cols) {
                forwardUp = false;
            } else if (grid[lastDropRow][lastDropCol] == grid[k][l]) {
                diagCounterTwo++;
                if (diagCounterTwo == 4) {
                    return true;
                }
        ...
        } //close check forward up
        while (reverseDown && (diagCounterTwo < 4)) {
            if (m == this.rows || n < 0) {
                reverseDown = false;
            } else if (grid[lastDropRow][lastDropCol] == grid[m][n]) {
                diagCounterTwo++;
                if (diagCounterTwo == 4) {
                    return true;
                }
